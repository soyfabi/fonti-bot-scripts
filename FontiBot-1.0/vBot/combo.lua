...
local serverTargetLastUpdate = 0
macro(10, function()
...
macro(10, function()
  ...
  if config.enabled and config.serverEnabled and config.attack == "SERVER LEADER TARGET" and serverTarget then
    if serverTarget and (not getTarget() or getTarget():getName() ~= serverTarget:getName())
    then
...
macro(10, function()
  ...
  if BotServer._websocket and config.enabled and config.serverEnabled then
    ...
    if currentTarget and now - timeout > 500 then
      targetPos = currentTarget:getName()
      BotServer.send("target", targetPos)
      lastSentTarget = targetPos
      timeout = now
    elseif not currentTarget and lastSentTarget ~= false and now - timeout > 500 then
      BotServer.send("target", "stop")
      lastSentTarget = false
      timeout = now
    end
  end
end)
  ...
  BotServer.listen("target", function(name, message)
    if name:lower() ~= player:getName():lower() and name:lower() == config.serverLeader:lower() then
      if message == "stop" then
        serverTarget = nil
        if config.serverLeaderTarget and config.attack == "SERVER LEADER TARGET" then
          g_game.cancelAttack()
        end
        return
      end
      ...
      if config.serverLeaderTarget and creature then
        serverTarget = creature
        if config.attack == "SERVER LEADER TARGET" then
