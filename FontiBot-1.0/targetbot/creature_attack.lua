...
function getWalkableTilesCount(position)
  ...
  for i, tile in pairs(getNearTiles(position)) do
      if tile:isWalkable() or #tile:getCreatures() > 0 then
          count = count + 1
      end
  end
...
function rePosition(minTiles)
  ...
  for i, tile in ipairs(tiles) do
      tilesTable[tile] = #tile:getCreatures() == 0 and tile:isWalkable() and getWalkableTilesCount(tile:getPosition()) or nil
  end
